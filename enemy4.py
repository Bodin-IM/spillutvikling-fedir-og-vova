...
class Enemy:
    ...
    def move_towards_hero(self, hero_x, hero_y):
        if self.state == "moving":
            if self.x < hero_x:
                self.x += self.speed * 0.7  # Увеличенная скорость
    ...
    def shoot(self):
        if self.state == "shooting":
            current_time = time.time()
            if current_time - self.shoot_animation_time > 0.2:  # Очередь каждые 200 мс
                ...
                self.burst_count += 1
                if self.burst_count >= 3:  # Завершение очереди
                    self.burst_count = 0
                    self.state = "moving"
    ...
    def update_state(self):
        ...
        current_time = time.time()
        if current_time - self.last_state_change > self.state_change_interval and self.state != "shooting":
...
# Создание врага
enemy = Enemy(WIDTH + 50, random.randint(0, HEIGHT - 50), 50, 3)
